...
Then(/^I should see the offer which title contains "(.*?)"$/) do |title|
  assert_on_table_row_and_column 2, 1, title
end

Then(/^I should see the offer which description contains "(.*?)"$/) do |description|
  assert_on_table_row_and_column 3, 3, description
end
